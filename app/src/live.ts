...
export async function liveAction(data:Data,_userId:string|null,action:string,p:any){
 ...
 switch(action){
 case 'startRmReview':{const documentId=await rpc('start_rm_review',{p_target:p.targetId});p.result={documentId};return}
 ...
 if(doc?.voluntaryReview)return rpc('save_voluntary_review',{p_document:doc.id,p_content:content,p_revision:doc.draftRevision??null,p_submit:false})
 ...
 case 'submitDocument':{
 if(doc?.voluntaryReview)return rpc('save_voluntary_review',{p_document:doc.id,p_content:content,p_revision:doc.draftRevision??null,p_submit:true})
