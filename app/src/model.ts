...
export type Initiative = { id: string; title: string; abstract: string; leadId: string; members: string[]; status: string; category: string; hp: number; tasks: {id:string;title:string;done:boolean}[]; motivation?: string; coverObjectPath?: string; coverFallbackColor?: string; coverUrl?: string }
export type DocumentRecord = { id:string; initiativeId:string; kind:'rm'|'review'; title:string; authorId:string; authorName?:string; status:string; body:string; version:number; submittedAt?:string; targetId?:string; historical?:boolean; sourceKey?:string; sourcePeriod?:string; sourcePeriodKey?:string; sourceWeek?:string; sourceOrder?:number; versions:{version:number;body:string;at:string}[] }
